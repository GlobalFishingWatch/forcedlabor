...
#' @export
#'
dev_ml_train3 <- function(cv_setup,
                          rf_spec,
                         free_cores,
                         new_data = NULL,
                         parallel_plan,
 ...
 seed = purrr::map(cv_setup, "seed"),
 bag = purrr::map(cv_setup, "bag"),
 recipe = purrr::map(cv_setup, "recipe"),
 folds_tbl = purrr::map(cv_setup, "cv_folds")
 ))
...
out <- furrr::future_map(
  ...
  .f = function(x) { #bag, recipe, folds_tbl) {
    seed <- x$seed
    bag <- x$bag
    recipe <- x$recipe
    ...
    return(list(
      pred_assess = purrr::map(out2, "pred_assess"),
      pred_new = purrr::map(out2, "pred_new")
    ))
  }
)
