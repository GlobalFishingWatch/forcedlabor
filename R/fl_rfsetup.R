...
#' @param y Character, name of the response variable. Defaults to
#'  `"known_offender"`
#' @param x Vector with names of the features to be used. Defaults to NULL and
...
#' @param corr_threshold A value for the threshold of absolute correlation
#' values. The step will try to remove the
#' minimum number of columns so that all the resulting absolute correlations are
#' less than this value. See [recipes::step_corr()] for further information
#' @param rf_trees Number of trees contained in the ensemble. The larger the
#' number of trees, the more stable the predictions - but with a higher
#' computational cost
#' @param rf_mtry Number of features that will be sampled at each split
#' @param rf_min_n Minimum node size (i.e. the number of vessel-years that a
#' final node should have). Larger values create less complex trees, and may
#' result in underfitting. Smaller values create more complex trees, and may
#' result in overfitting
#' @param rf_reg_factor Regularization factor works by penalizing new variables
#' by multiplying the splitting criterion by a factor. As a result, the tree is
#' biased to keep splitting on features it already trusts, rather than
