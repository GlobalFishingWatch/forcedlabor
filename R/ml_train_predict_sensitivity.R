...
ml_hyperpar_sensitivity <- function(train_pred_proba) {
...
ml_frankenstraining_sensitivity <- function(training_df, fl_rec, rf_spec, cv_splits_all,
  ...
  dplyr::mutate(
    prediction_output = furrr::future_map(.x = .data$counter, .f = function(x) {
      ...
      seed_counter <- bag_runs$common_seed[bag_runs$counter == x]
      bag_group_counter <- bag_runs$bag_group[bag_runs$counter == x]

      # Ensure all bags look the same
      set.seed(seed_counter)
      # get the 10 folds related to that common seed
      cv_splits <- cv_splits_all %>%
        dplyr::filter(.data$common_seed == seed_counter) %>%
        .$cv_splits %>%
        purrr::pluck(1)
      ...
      best_hyperparameters_temp <- best_hyperparameters$best_hyperparameters %>%
        dplyr::filter(.data$common_seed == seed_counter &
                        .data$bag_group == bag_group_counter)
